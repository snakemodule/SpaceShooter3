...
void systems::enemy_attack::register_archetype(Archetype& matching_archetype)
{
...
void systems::enemy_attack::shoot(float dt, Manager& manager)
{	
	...
	for (size_t i = 0; i < archetype_count; i++)
	{
		...
		{			
			...
			if (timer_data[j].timer <= 0)
			{
				timer_data[j].timer = game_data::enemy_properties.attack_delay;
				...
				velocity_data_created[record.component_idx].y = game_data::enemy_properties.bullet_speed;
			}
		}
	}
}
