...
void systems::enemy_collision::register_archetype(Archetype& matching_archetype)
{
...
void systems::enemy_collision::collide(float dt, Manager& manager)
{
	Archetype& bullets = manager.get_archetype(systems::player_attack::bullet_create_sig);
	Position* player_bullet_positions = get_data_of_component_array<Position>(bullets);
	...
	for (size_t i = 0; i < archetype_count; i++)
	{
		...
		{
			const float player_bullet_radius = game_data::player_properties.bullet_size / 2;
			const float enemy_radius = game_data::enemy_properties.size / 2;
			const float collision_distance = player_bullet_radius + enemy_radius;
			...
			for (int x = xminbucket; x <= xmaxbucket; x++)
			{
				for (int y = yminbucket; y <= ymaxbucket; y++)
				{
					...
					key |= y;
					auto range = indexing::space_partition::player_bullet_position_map.equal_range(key);
					for (auto it = range.first; it != range.second; ++it) 
					{
						Entity& bullet_entity = it->second;
						unsigned bullet_idx = manager.records[bullet_entity.index()].component_idx;
						Position bullet_pos = player_bullet_positions[bullet_idx];
